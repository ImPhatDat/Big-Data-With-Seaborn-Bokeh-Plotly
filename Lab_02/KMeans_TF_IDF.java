...
public class KMeans_TF_IDF {
    ...
    // Unclustered
    public static List<TF_IDF_Vector> initializeCentroids(int k, FileSystem fs, String inputPath, List<String> terms)
            throws IOException {
        ...
        for (FileStatus file : files) {
            try (BufferedReader br = new BufferedReader(new InputStreamReader(fs.open(file.getPath())))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (empty_count != 0) {
                        empty_count--;
                        vectors.get(empty_count).add(currentVector);
                    } else {
                        int clusterIndex = (int) (Math.random() * k);
                        vectors.get(clusterIndex).add(currentVector);
                    }
                }
                br.close();
            }
        ...
        for (int i = 0; i < k; i++) {
            centroids.add(i, getCentroid(vectors.get(i), terms));
        }
        return centroids;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(KMeans_TF_IDF.class);
        ...
        List<TF_IDF_Vector> centroids = initializeCentroids(k, fs, transformed_file, terms);
